...
class Train_Network(object):
    def __init__(self, encoder, decoder, index2word, num_layers=1, teacher_forcing_ratio=0.0):
    ...
    def evaluate(self, input_variables, target_variables, lengths, criterion):
        ''' Pad all tensors in this batch to same length. '''
        input_variables = torch.nn.utils.rnn.pad_sequence(input_variables)
        target_variables = torch.nn.utils.rnn.pad_sequence(target_variables)
    ...
    def get_response(self, input_variable):
        ''' Convert to Tensor. '''
